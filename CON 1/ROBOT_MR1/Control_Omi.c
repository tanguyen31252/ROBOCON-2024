...
void RUN_XOAY_TRON(vs32 _toc_do,float _gia_toc)		
		{				BIT_XOAY		=	10;
						...
						if(HUONG>0){chieu_trai_lui;chieu_phai_lui;chieu_sau_phai;}
						else if(HUONG<0){chieu_trai_toi;chieu_phai_toi;chieu_sau_trai;}
}	                 //       
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx	
void TINH_GOC_LECH()		
{				
						
							GOC_LECH=(IMU-MAT_ROBOT)/20; // do dap ung
	
						trai =	(int)(luong_giac_trai*TOC_DO  + GOC_LECH);
						phai =	(int)(luong_giac_phai*TOC_DO  + GOC_LECH);	
						sau  =	(int)(luong_giac_sau*TOC_DO  + GOC_LECH);	

							if(trai>0){ chieu_trai_lui;}
							else {chieu_trai_toi;} 
							
							if(phai<0){ chieu_phai_toi;}
							else {chieu_phai_lui;} 

							if(sau<0){ chieu_sau_trai;}
							else {chieu_sau_phai;} 							
