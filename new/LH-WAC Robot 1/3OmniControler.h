...
{
	if(T == 0) 			return 0;
	else 	if(T < 0) return (T*0.3 - 0.3)/_robotCurrentSpeed; // tuy thuoc robot ma co the he so toi thieu la 0.5, 1, 2...
				else 			return (T*0.3 + 0.3)/_robotCurrentSpeed;
}
//------------------------------------------------------------------------------
        ...
        if(_robotCurrentSpeed != _robotRunSpeed)
        {
            if(_robotCurrentSpeed < _robotRunSpeed)
            {
            	if(_robotCurrentSpeed < _robotMaxSpeed) ac_SpeedACC(giaToc(_robotCurrentSpeed), 2); //--- Phuong trinh Gia toc --------	
						...
						else ac_SpeedACC(_robotDelayGiamToc, -1.5); //--- Phuong trinh Giam toc --------
        }
    }
