...
void bat_cam()
{
    while(vi_tri_lech != 22)    //khac vi tri trung tam
    {
        if(vi_tri_lech == 11)   //lech trai
        {
            robotRunAngle(-900, 10, 0, 2);
        }
        if(vi_tri_lech == 33)   //lech phai
        {
            robotRunAngle(900, 10, 0, 2);
        }
        if(vi_tri_lech == 44)   //khong co bong hoac mat tin hieu
        {
            
        }
        if(vi_tri_lech == 22)   //trung tam
        {
...
*/
        ...
        {
            robotRun(850, 25);
            while(laze_8H_Value < do_laze_silo-20){if(wantExit())break;}
...
/****************************************SAN XANH****************************************/
    else
    {
        ...
        {
            robotRun(-850, 25);
            while(laze_4H_Value < do_laze_silo-20){if(wantExit())break;}
...
void test()
{
    ...
    // while(bien_nho_bong_trong_silo[0] != 3)
    // {
    //     do_bong();
    ...
    // while(laze_0H_Value < 150)
    // {
    //     if(NUT_CHUYEN_SAN == 0)
    //     {
    //         bam_thanh_laze_ngang(1800, 50, 0, 2, laze_silo[NUT_CHUYEN_SAN][3], 1);
    //     }
    //     else
    //     {
    //         bam_thanh_laze_ngang(1800, 50, 0, 2, laze_silo[NUT_CHUYEN_SAN][3], 2);
    //     }
    // }
...
void restartDo(){
    ...
    for(i=0;i<50;i++) while(laze_8H_Value > 55){vTaskDelay(1);}
    robotRunAngle(0, 60, 0, 0.5);
    RESET_ENCODER();
    for(i=0;i<50;i++) while(ENCODER_TONG() < 3900){vTaskDelay(1);}
    robotRunAngle(0, 20, 0, 0.5);
    for(i=0;i<50;i++) while(laze_0H_Value > 153){vTaskDelay(1);}
    RESET_ENCODER();
    robotRunAngle(-900, 60, 0, 0.5);
    for(i=0;i<50;i++) while(ENCODER_TONG() < 5800){vTaskDelay(1);}

    robotRunAngle(-900, 40, 0, 0.5);
    for(i=0;i<50;i++) while(laze_8H_Value > 175){vTaskDelay(1);}
...
check_bong:
    while(GP_BTN[1] != 22){
        if(GP_BTN[1] == 33) robotRunAngle(-900, 20, 0, -0.5);
        else if(GP_BTN[1] == 11) robotRunAngle(900, 20, 0, 0.5);
        if(laze_4H_Value < 50) {
            XL_NONG_XOAY_BAT;
            vTaskDelay(1000);
        }else{
    ...
    {   XL_MO_90;
        ...
        Mor_hot_bong = 200, Bang_tai = Mor_nong = 254;
        Mor_hot_bong_vao, Bang_tai_thuan, Mor_nong_ra;
        if(CB_NHAN_DO_TREN == 0 || CB_NHAN_XANH_TREN == 0)  {bien_nhan_bong = 1; Mor_hot_bong =Bang_tai=0;break;}
